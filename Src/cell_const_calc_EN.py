...
def analyze_pulse(pulse):
    ...
    t=pulse["Step Time"].to_numpy()
    ocv=pulse["ocv[v]"].iloc[0]
    V=pulse["Voltage [V]"].to_numpy()
    I=np.mean(pulse["Current [A]"])
    Tb= np.mean(pulse["Battery Temperature [°C]"])
    Ta= np.mean(pulse["Chamber Temperature [°C]"])
...
#    initialdir=cwd, #現在のディレクトリ(VSCODE)
    initialdir=csv_path,
    title="Please select the file for cell const. analysis.",
...
for soc in sorted(df["soc"].unique()):  #最初に現れたsoc値を抽出する（重複は選ばない) sortedは昇順
    ...
    for i in range(1,len(df_soc)):
        if df_soc["Step Time"].iloc[i]<df_soc["Step Time"].iloc[i-1]:     #時間が小さくなったら、、、
